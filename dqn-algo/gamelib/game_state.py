...
class GameState:
    ...
    def __parse_state(self, state_line):
        ...
        p1_health, p1_cores, p1_bits, p1_time = map(
            float, state["p1Stats"][:4])
        p2_health, p2_cores, p2_bits, p2_time = map(
            float, state["p2Stats"][:4])
    ...
    def get_resource(self, resource_type, player_index=0):
        ...
        if not resource_type == self.BITS and not resource_type == self.CORES:
            self.warn("Invalid resource_type '{}'. Please use BITS (0) or CORES (1)".format(
                resource_type))
            return
    ...
    def project_future_bits(self, turns_in_future=1, player_index=0, current_bits=None):
        ...
        if type(current_bits) == int and current_bits < 0:
            self.warn("Invalid current bits ({}). Current bits cannot be negative.".format(
                current_bits))

        bits = self.get_resource(
            self.BITS, player_index) if not current_bits else current_bits
        for increment in range(1, turns_in_future + 1):
            ...
            bits_gained = self.config["resources"]["bitsPerRound"] + (self.config["resources"]["bitGrowthRate"] * (
    ...
    def attempt_remove(self, locations):
        ...
        for location in locations:
            if location[1] < self.HALF_ARENA and self.contains_stationary_unit(location):
                ...
                self.warn(
                    "Could not remove a unit from {}. Location has no firewall or is enemy territory.".format(location))
    ...
    def attempt_upgrade(self, locations):
        ...
        for location in locations:
            if location[1] < self.HALF_ARENA and self.contains_stationary_unit(location):
                ...
                self.warn(
                    "Could not upgrade a unit from {}. Location has no firewall or is enemy territory.".format(location))
        return spawned_units
