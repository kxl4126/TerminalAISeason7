...
import pickle
import numpy as np
from preprocessing import scale_other_stats, powerset
...
class RLTemplateAlgo(gamelib.AlgoCore):
    def __init__(self):
        super().__init__()
    ...
    def on_turn(self, turn_state):
        ...
        processed_game_state = self.get_processed_game_state(game_json)
        action = [0] * (len(self.ATTACK_OPTIONS) + len(self.DEFENSE_OPTIONS))
        ...
        random_action = '{0:06b}'.format(random.randint(0, len(action) - 1))
        random_action = [int(x) for x in list(random_action)]
        assert len(random_action) == 6
        # action = random_action
        action = [1, 1, 1, 0, 1, 1]  # can add how much money to spend later
        for i in range(len(action)-2, len(action)):
            action[i] = random.randint(0, 1)
        reward = 0
        # arbitrary multiplier so we spend more when we are dying
        do_action(action, game_state, game_json)
        # choose action and see reward
        game_state.submit_turn()
        return processed_game_state, action, reward
    ...
    def get_processed_game_state(self, game_json):
        ...
        other_stats = self.process_other_stats(game_json)
    ...
    def process_other_stats(self, game_json):
        p1_stats_dict = {
            'health': game_json['p1Stats'][HEALTH_INDEX],
            'sp': game_json['p1Stats'][SP_INDEX],
            'mp': game_json['p1Stats'][MP_INDEX],
            'turn': game_state.turn_number
        }
        p1_stats_scaled = scale_other_stats(p1_stats_dict)
        p2_stats_dict = {
            'health': game_json['p2Stats'][HEALTH_INDEX],
            'sp': game_json['p2Stats'][SP_INDEX],
            'mp': game_json['p2Stats'][MP_INDEX],
            'turn': game_state.turn_number
        }
