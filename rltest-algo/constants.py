...
ATTACK_LEFT_COORDS = [[i, 13-i] for i in range(14)]
ATTACK_LEFT_COORDS.extend([[i, 14-i] for i in range(1, 14)])
ATTACK_RIGHT_COORDS = [[14+i, i] for i in range(14)]
ATTACK_RIGHT_COORDS.extend([[13+i, i] for i in range(1, 14)])


# do i want to let my structures be placed on edges?
TOP_LEFT_COORDS = [list(coord) for coord in coords if coord[0] >=
                   0 and coord[0] <= 7 and coord[1] >= 6 and coord[1] <= 13 and list(coord) not in ATTACK_LEFT_COORDS and list(coord) not in ATTACK_RIGHT_COORDS]
TOP_MIDDLE_COORDS = [list(coord) for coord in coords if coord[0] >=
                     8 and coord[0] <= 19 and coord[1] >= 6 and coord[1] <= 13 and list(coord) not in ATTACK_LEFT_COORDS and list(coord) not in ATTACK_RIGHT_COORDS]
TOP_RIGHT_COORDS = [list(coord) for coord in coords if coord[0] >=
                    20 and coord[0] <= 27 and coord[1] >= 6 and coord[1] <= 13 and list(coord) not in ATTACK_LEFT_COORDS and list(coord) not in ATTACK_RIGHT_COORDS]
BOTTOM_COORDS = [list(coord) for coord in coords if coord[0] >=
                 8 and coord[0] <= 19 and coord[1] >= 0 and coord[1] <= 5 and list(coord) not in ATTACK_LEFT_COORDS and list(coord) not in ATTACK_RIGHT_COORDS]
